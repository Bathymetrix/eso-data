...
HDOP	log_gps_records	nearest unused gps_record_kind == "dop" record from the same source file at or after the LOG GPS anchor and within the configured forward DOP window; raw_values.hdop	DOP observations are consumed at most once and are never assigned backward to a later GPS anchor.
...
The DOP window is directional: a DOP observation must be simultaneous with or
later than its LOG GPS anchor. Battery, pressure, and Iridium windows remain
symmetric around their anchors.
