...
Datetime	log_gps_records.<serial>.jsonl	record_time where gps_record_kind == "fix_position"	Convert ISO UTC to 08-Oct-2018 10:07:48 format only in the table writer.
Latitude decimal degrees	log_gps_records	raw_values.latitude where gps_record_kind == "fix_position"	Convert the source-literal coordinate (e.g. S14deg41.862mn) to decimal degrees in eso-data; normalization intentionally preserves the original string.
Longitude decimal degrees	log_gps_records	raw_values.longitude where gps_record_kind == "fix_position"	Same conversion policy as latitude.
HDOP	log_gps_records	nearest unused gps_record_kind == "dop" record from the same source file; raw_values.hdop	DOP observations are consumed at most once and assigned to the nearest GPS anchor.
...
Files queued for upload	unavailable in current normalized records	no normalized field	Written as missing (-1) until queue-count telemetry is exposed by mermaid-records.
...
GPS fix_position observations define the output rows.
